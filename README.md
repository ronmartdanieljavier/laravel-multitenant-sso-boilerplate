...
│   ├── Services/
│   │   └── DocumentService.php          # paginate(), list(), find(), store(), downloadResponse(), downloadZipResponse(), delete(), getFilePath()
│   └── Tests/
...
- **Functional tenant dashboard (Phase 6)** — `Tenant/Index.vue` is a live data-driven dashboard with four stats cards (active reports, failed reports, open errors, total documents), three quick-action links (Queue Report / Browse Documents / Review Errors), and three recent-activity sections (up to 5 recent reports, unresolved errors, and documents); fully backed by `TenantDashboardService` → `ReportRepository::summaryForTenant()` / `recentForTenant()`, `TenantErrorLogRepository::listForTenant()`, and `DocumentRepository::countAll()` / `recentList()`; API at `GET /api/v1/tenant/dashboard`; 38 Vitest + 31 PHPUnit tests; fixed a silent production bug where all stats displayed as 0 due to camelCase vs. snake_case mismatch between Vue and Spatie Data
...
**Tenant Dashboard (API — Bearer + X-App + X-Tenant headers)**

| Method | Endpoint | Auth | Description |
|---|---|---|---|
| `GET` | `/api/v1/tenant/dashboard` | Bearer | Live dashboard snapshot — stats (pending/processing/failed reports, success this month, unresolved/critical errors, total documents) plus 5 most recent reports, errors, and documents |
