...
class ResolveWebTenantDatabase
{
    ...
    private function configureConnection(Tenant $tenant): void
    {
        ...
        if ($tenant->hasReadReplica()) {
            $connection['write'] = ['host' => $tenant->db_host, 'port' => $tenant->db_port ?? 3306];
            $connection['read'] = ['host' => $tenant->read_replica_host, 'port' => $tenant->read_replica_port ?? $tenant->db_port ?? 3306];
