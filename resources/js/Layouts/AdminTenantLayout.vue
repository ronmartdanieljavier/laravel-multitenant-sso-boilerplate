...
const tenantNav = computed(() => {
    ...
    if (!id) { return []; }
    return [
        { label: 'Settings', href: `/admin/tenants/${id}/settings`, color: 'violet' },
        { label: 'Users',    href: `/admin/tenants/${id}/users`,    color: 'slate' },
        { label: 'Reports',  href: `/admin/tenants/${id}/reports`,  color: 'blue' },
        { label: 'Errors',   href: `/admin/tenants/${id}/errors`,   color: 'red' },
    ];
});
...
const hoverColors = {
    violet: 'hover:bg-violet-600/10 hover:text-violet-300',
    slate:  'hover:bg-slate-700 hover:text-slate-200',
    ...
    <div class="min-h-screen bg-slate-950 text-slate-100 flex">
        ...
        <aside class="fixed inset-y-0 left-0 w-60 bg-slate-900 border-r border-white/5 flex flex-col z-20">
            <div class="h-16 flex items-center px-6 border-b border-white/5 shrink-0">
                <div class="w-8 h-8 bg-violet-600 rounded-lg flex items-center justify-center mr-3 shrink-0">
            ...
            <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                ...
                <Link href="/admin/tenants" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition bg-violet-600/20 text-violet-300">Tenants</Link>
